...
CERT_DETAILS_CACHE_TIMEOUT = 60*60  # 1h
...
def get_certification_details(uid):
    ...
    # Handle missing cert
    if cert_url is None:
        logger.error(f"Sought cert uid '{uid}' was not found in catalog")
        return None

    # Check if previously cached -> if so return cached version
    cached_cert_details = cache.get(f'cert-{uid}')
    if cached_cert_details is not None:
        return json.loads(cached_cert_details)

    # Retrieve details
    r = requests.get(urllib.parse.urljoin("https://docs.microsoft.com/en-us/", cert_url))
    ...
    cache.set(f'cert-{uid}', json.dumps(cert_details), timeout=CERT_DETAILS_CACHE_TIMEOUT)
    return cert_details
